...
class NewCourseEntryTest(LiveServerTestCase):
    ...
    def test_form_displays_correctly(self):
        ...
        self.assertIn('Add Or Edit Course', self.browser.title)
        header_text = self.browser.find_element_by_tag_name('h2').text
        self.assertIn('Add Or Edit Course', header_text)
        ...
        # A save button at the bottom adds the data to the system
    ...
    def test_can_edit_existing_course(self):
        ...
        course_id_edit = self.browser.find_element_by_id("course_edit_id").get_attribute("href")[-5:]

        # The form is repopulated with the values for the course that he wants to edit.
        courses = Course.objects.all()
        courseToEdit = courses.filter(course_id=course_id_edit).values()

        course_id_2 = self.browser.find_element_by_id('course_id')
        course_id_2.clear()
        course_id_2.send_keys(courseToEdit[0]['course_id'])
